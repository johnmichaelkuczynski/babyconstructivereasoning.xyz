...
**Diagnostic seed is a SEPARATE seed with its own self-heal:** the four-phase diagnostic assessments (`seedDiagnostics.ts`) are seeded independently of the curriculum and originally keyed their reseed ONLY on the `instrument:phase` set, so editing diagnostic prompts/instructions never propagated to existing DBs. Fix: fold a content hash (instructions + ordered prompts; exclude rotated options) into both `desiredSignature` and `existingSignature` so any prompt edit triggers a replace. `existingSignature` must filter to template items via `i.attempt_id IS NULL`, or attempt-generated items pollute the hash and cause a false reseed loop.
