...
export function setupAuth(app: Express) {
  ...
  if (googleEnabled) {
    ...
    // registered in the owner's Google Cloud Console OAuth client. The /api
    // prefix is required in this app because the shared proxy routes only
    // /api paths to this server.
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        return `https://${prodDomain || "basicconstructivereasoning.xyz"}${CALLBACK_PATH}`;
      }
    ...
    const trustedHosts = new Set<string>(
      [
        ...
        "basicconstructivereasoning.xyz",
        "www.basicconstructivereasoning.xyz",
        "babyconstructivereasoning.xyz",
        "www.babyconstructivereasoning.xyz",
        "baby-constructive-reasoning.replit.app",
        "www.baby-constructive-reasoning.replit.app",
        "localhost:80",
      ]
        .filter(Boolean)
        .map((h) => h.toLowerCase())
    ...
    // Legacy alias in case the un-prefixed URI is registered instead
